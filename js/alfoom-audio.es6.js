...
const i = ~~document.cookie.split('~')[1] // 0 - 3
    , worker = new Worker('../js/seqin-worker.es6.js')

//// Create a Seqin instance.
demo1 = window.DEMO = new SEQIN.Main({
    worker:   worker // runs a metronome, outside of the main thread
  , tracks:   3      // number of audio channels (default 2)
  , steps:    32     // defines the grid (default 16)
  , fidelity: 44100   // samples per step (default 5400)
})

// console.log(demo1.ctx.sampleRate);
//// Listen for events.
